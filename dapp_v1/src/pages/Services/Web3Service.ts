...
export async function addBook(book: Book) {
  ...
  return contract.methods.addBook(book.title,book.year).send();
}

export async function getLastBooks(page:any) {
  const contract = getContract();
  const books = await contract.methods.getLastBooks(page).call();
